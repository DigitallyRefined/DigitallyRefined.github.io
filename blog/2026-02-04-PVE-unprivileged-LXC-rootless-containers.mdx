...
Now we can run the [PVE LXC Unprivileged Rootless Docker script](https://github.com/DigitallyRefined/scripts/releases/latest/download/pve-unprivileged-lxc-rootless-containers.sh):

```bash
bash -c "$(curl -fsSL https://github.com/DigitallyRefined/scripts/releases/latest/download/pve-unprivileged-lxc-rootless-containers.sh)"
```

:::warning
Please [review the script](https://github.com/DigitallyRefined/scripts/releases/latest/download/pve-unprivileged-lxc-rootless-containers.sh) before running it before running it on your system.
...
services:
  traefik:
    ...
    labels:
      ...
      traefik.http.routers.traefik-wildcard.entrypoints: web,websecure
      traefik.http.routers.traefik-wildcard.rule: Host(`example.dedyn.io`)
      traefik.http.routers.traefik-wildcard.tls: true
      traefik.http.routers.traefik-wildcard.tls.certresolver: production-desec-dns
      traefik.http.routers.traefik-wildcard.tls.domains[0].main: "*.example.dedyn.io"
      traefik.http.routers.traefik-wildcard.tls.domains[0].sans: "example.dedyn.io"
      traefik.http.routers.traefik-wildcard.service: noop@internal
...
`traefik/data/config/traefik.yml`
```yaml
entryPoints:
  web:
    address: :80
...
- [PVE LXC Unprivileged Rootless Docker - pve-unprivileged-lxc-rootless-containers.sh](https://github.com/DigitallyRefined/scripts/releases/latest/download/pve-unprivileged-lxc-rootless-containers.sh)
